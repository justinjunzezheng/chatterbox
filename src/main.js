...
async function start(supabase) {
  ...
  function renderApp() {
    ...
    root.innerHTML = `<main class="messenger"><nav class="rail"><div class="logo">c</div><div class="nav-icons"><button class="active" title="Messages">◫</button><button id="groups" title="Create group">♙</button><button id="settings" title="Settings">⚙</button></div><button id="profile" class="avatar me" title="Edit profile">${initials(name)}</button></nav><aside class="inbox"><header><div><p class="eyebrow">Your inbox</p><h2>Messages</h2></div><button id="find" class="new" title="Find people">＋</button></header><div class="search"><span>⌕</span><input id="filter" placeholder="Search your chats" /></div><div id="chat-list" class="chat-list"></div></aside><section id="conversation" class="conversation"></section></main><div id="modal"></div>`;
